<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="LoanTotal.aspx.cs" Inherits="WebUI.ReportStatistics.LoanTotal" %>
...
<meta http-equiv="Content-Type" content="text/html; charset=utf-8"/>
    ...
    <script type="text/javascript">
        $(function () {
            ...
            //数据源
            var source = {
                url: '/HanderAshx/ReportStatistics/LoanTotalHandler.ashx',
                cache: false,
                datatype: "json",
                root: 'Rows',
                datafields: [
                    { name: 'ID', type: 'int' },
                    { name: 'LoanTypeName', type: 'string' },
                    { name: 'DealAmount', type: 'number' },
                    { name: 'ReceivedInterest', type: 'number' },
                    { name: 'AvgRate', type: 'number' },
                    { name: 'ReInterest', type: 'number' },
                    { name: 'OverReInterest', type: 'number' },
                    { name: 'ReceivedLoanCharges', type: 'number' },
                    { name: 'ReLoanCharges', type: 'number' }
                ],
                pagesize: 20,
            ...
            $("#jqxgrid").jqxGrid({
                ...
                columns: [
                    { dataField: 'LoanTypeName', text: '<b>借款标类型</b>', width: 140, cellsalign: 'center', align: 'center' },
                    { dataField: 'DealAmount', text: '<b>成交总额(元)</b>', width: 140, cellsalign: 'center', align: 'center', cellsformat: "F2", aggregates: ['sum'] },
                    { dataField: 'ReceivedInterest', text: '<b>已获利息(元)</b>', width: 140, cellsalign: 'center', align: 'center', cellsformat: "F2", aggregates: ['sum'] },
                    { dataField: 'AvgRate', text: '<b>平均年化率</b>', width: 70, cellsalign: 'center', align: 'center', cellsformat: "F2" },
                    { dataField: 'ReInterest', text: '<b>待收利息(元)</b>', width: 140, cellsalign: 'right', align: 'center', cellsformat: "F2", aggregates: ['sum'] },
                    { dataField: 'OverReInterest', text: '<b>逾期待收利息(元)</b>', width: 140, cellsalign: 'center', align: 'center', cellsformat: "F2", aggregates: ['sum'] },
                    { dataField: 'ReceivedLoanCharges', text: '<b>已获管理费(元)</b>', width: 140, cellsalign: 'right', align: 'center', cellsformat: "F2", aggregates: ['sum'] },
                    { dataField: 'ReLoanCharges', text: '<b>待获管理费(元)</b>', width: 140, cellsalign: 'center', align: 'center', cellsformat: "F2", aggregates: ['sum'] }
                ]
            });
