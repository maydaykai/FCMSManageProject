...
namespace WebUI.HanderAshx.p2p
{
    ...
    /// AbandonLoan 的摘要说明
    /// </summary>
    public class AbandonLoan : IHttpHandler
    {
        ...
        public object GetPageList(int pagenum, int pagesize, string sortField, string filter)
        {
            pagenum = pagenum + 1;
            int total;
            var abandonLoanBLL = new ApplyAbandonLoanBLL();
            ...
            var dt = abandonLoanBLL.GetPageList(fields, filter, sortField, pagenum, pagesize, out total);
